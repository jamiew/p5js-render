...
function updateSeeds(time) {
  for (let seed of seeds) {
    ...
    seed.x =
      seed.baseX +
      sin(t * 1.2) * seed.radius +
      cos(t * 2.3) * seed.radius * 0.5;
    seed.y =
      seed.baseY +
      cos(t * 1.1) * seed.radius +
      sin(t * 1.8) * seed.radius * 0.6;
    ...
    for (let y = 0; y < height; y += step) {
      ...
      if (abs(distToClosest - distToSecond) < 8) {
        shards.push({
          ...
          angle: atan2(
            secondClosest.y - closestSeed.y,
            secondClosest.x - closestSeed.x
          )
        });
      }
    }
...
function drawShards(time) {
  for (let shard of shards) {
    ...
    let strobePhase =
      sin(time * 8 + shard.x * 0.01 + shard.y * 0.01) * 0.5 + 0.5;
...
function drawKaleidoscopeSegment(time, segment) {
  ...
  for (let i = 0; i < numLines; i++) {
    ...
    if (strobePhase > 0.7) {
      ...
      line(
        x2 - cos(perpAngle) * spread,
        y2 - sin(perpAngle) * spread,
        x2 + cos(perpAngle) * spread,
        y2 + sin(perpAngle) * spread
      );
    }
  ...
  for (let i = 0; i < numShards; i++) {
    ...
    vertex(
      cos(shardAngle + 0.1) * outerRadius,
      sin(shardAngle + 0.1) * outerRadius
    );
    vertex(
      cos(shardAngle - 0.1) * outerRadius,
      sin(shardAngle - 0.1) * outerRadius
