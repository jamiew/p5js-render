...
function draw() {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    let x =
      width / 2 + sin(particleTime + i) * (150 + sin(particleTime * 2) * 100);
    let y =
      height / 2 +
      cos(particleTime * 1.3 + i * 0.7) * (120 + cos(particleTime) * 80);
    ...
    circle(
      x - cos(particleTime + i) * 5,
      y - sin(particleTime + i) * 5,
      size * 0.7
    );
  }
  ...
  for (let x = 0; x < width; x += 4) {
    let waveY =
      height / 2 +
      sin(x * 0.02 + time * 3) * 100 +
      cos(x * 0.01 + time * 2) * 50;
    point(x, waveY);
  }
}
